...
public class SysUserController extends ResponseUtils {
    @Autowired
...
//
    ...
    @GetMapping("/current")
    public ResultObj getLoginAppUser(String token) {
        String user = tokenCache.getToken(token);
        return success(JSONObject.fromObject(user));
    }
//
//    /**
//     * 查询用户实体对象SysUser
...
//    }
    ...
    public WebPageResult<SysUser> findUsers(@RequestParam Map<String, Object> params) {
    ...
    public ResultObj updateEnabled(@RequestParam Map<String, Object> params) {
        String id = MapUtils.getString(params, "id");
        if (checkAdmin(id)) {
            return error(500,ADMIN_CHANGE_MSG);
        }
        appUserService.updateEnabled(params);
        return success();
    }
    ...
    public ResultObj resetPassword(@RequestParam String id,String token) {
        if (checkAdmin(id)) {
            return error(500,ADMIN_CHANGE_MSG);
        }
     ...
     * 用户自己修改密码
     */
    @PutMapping(value = "/xgpassword")
    public ResultObj resetPassword(@RequestBody SysUser sysUser) {
    ...
    public ResultObj saveOrUpdate(@RequestBody SysUser sysUser) throws Exception {
        appUserService.saveOrUpdateUser(sysUser);
        return success();
    }
